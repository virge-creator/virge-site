...
function updatePost(filePath, slug) {
  ...
  // Check if already has image and tags
  if (content.includes('image:') && content.includes('tags:')) {
  ...
  const closingDashIndex = lines.findIndex((line, idx) => idx > 0 && line.trim() === '---');
  
  if (closingDashIndex === -1) {
    console.log(`❌ Could not find frontmatter closing for ${slug}`);
    return;
  }
  
  // Insert image and tags before the closing ---
  const tagsStr = `tags: ${JSON.stringify(meta.tags)}`;
  const imageStr = `image: "${meta.image}"`;
  
  lines.splice(closingDashIndex, 0, imageStr);
  lines.splice(closingDashIndex + 1, 0, tagsStr);
...
console.log('Updating EN posts...');
enFiles.forEach(file => {
  const slug = file.replace('.md', '');
  updatePost(path.join(enDir, file), slug);
});
...
nlFiles.forEach(file => {
  const slug = file.replace('.md', '');
  updatePost(path.join(nlDir, file), slug);
});
